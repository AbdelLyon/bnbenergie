...
export function ChatBot() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 md:inset-auto md:bottom-24 md:right-6 z-50 flex w-full md:w-95 h-dvh md:h-130 flex-col overflow-hidden rounded-none md:rounded-lg border-0 md:border border-neutral-200 bg-white shadow-2xl shadow-black/20 transition-all duration-300 ${
          isOpen
      ...
      >
        <div className="flex items-center gap-3 border-b border-neutral-200 px-4 py-3 ">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-amber-200/30 border border-neutral-200">
          ...
          <button
            ...
            className="md:hidden flex items-center justify-center h-8 w-8 rounded-full hover:bg-neutral-100 transition-colors cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {messages.length === 0 && (
            <div className="space-y-3">
              <div className="rounded-lg bg-neutral-100 flex items-center border border-neutral-200 gap-2 px-3 py-2 text-sm">
              ...
              <div className="flex flex-col gap-2">
                {SUGGESTED_QUESTIONS.map((q) => (
                  <button
                    ...
                    className="text-left text-xs px-3 py-2 rounded-lg bg-amber-200/30  hover:bg-amber-200/60 border border-neutral-200 transition-colors duration-200 cursor-pointer"
                  >
                    {q}
                  </button>
                ))}
          ...
          {messages.map((m) => (
            ...
            >
              <div className="h-7 w-7 flex items-center justify-center rounded-full bg-neutral-200">
                {m.role === 'user' ? (
