...
export function FAQItem({
  ...
  const config =
    (category ? categoryConfig[category] : undefined) ?? defaultConfig;
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          {category && (
            <p
              className={`text-[11px] font-bold uppercase tracking-wider mb-0.5 ${config.labelColor} ${config.labelColorDark}`}
            >
              {category}
            </p>
          )}
          <Heading
            as="h3"
            className="text-sm font-medium text-neutral-900 dark:text-white leading-snug md:text-base pr-2"
          >
            {question}
          </Heading>
        </div>
      ...
      >
        <div
          className={`mx-5 mb-5 rounded-xl px-4 py-4 text-sm leading-relaxed text-neutral-700 dark:text-neutral-300 ${config.answerBg} ${config.answerBgDark}`}
        >
          {answer}
        </div>
      </div>
